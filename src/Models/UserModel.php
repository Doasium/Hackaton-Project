...
class UserModel
{
    ...
    public function contact($data) {
        // Verileri sanitize ediyoruz
        $name = htmlspecialchars(trim($data['Name']));
        $surname = htmlspecialchars(trim($data['surname']));
        $email = filter_var($data['Email'], FILTER_VALIDATE_EMAIL);
        $phoneNumber = htmlspecialchars(trim($data['PhoneNumber']));
        $category = intval($data['Category']);
        $title = htmlspecialchars(trim($data['title']));
        $message = htmlspecialchars(trim($data['content']));

        if (!$name || !$surname || !$email || !$phoneNumber || !$title || !$message) {
            return true;
        }
